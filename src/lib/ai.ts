 ...
 * Uses Groq (fast inference, free tier) or falls back to local responses.
 */
...
export async function getAdvisorResponse(
  ...
  // If Groq key is set, use real AI
  if (GROQ_API_KEY) {
    try {
      const contextMsg = context
        ? `\n\nCurrent data context: City: ${context.city ?? "Unknown"}, Temp: ${context.temperature ?? "N/A"}°F, Risk: ${context.riskLevel ?? "N/A"}, Humidity: ${context.humidity ?? "N/A"}%`
        : "";
...
function getLocalResponse(
  ...
  if (lower.includes("safe") || lower.includes("walk") || lower.includes("go outside")) {
    return `## 🌡️ Safety Assessment for ${city}\n\n` +
      (temp
        ? `Current temperature is **${Math.round(temp)}°F** (${Math.round(((temp - 32) * 5) / 9)}°C).\n\n`
        : "") +
      `### Recommendations:\n` +
  ...
  if (lower.includes("route") || lower.includes("cool") || lower.includes("path")) {
    return `## 🗺️ Cool Route Tips for ${city}\n\n` +
      ...
      `5. **Check our Route Planner** for AI-optimized cool paths\n\n` +
      `*Use the Cool Route Planner for real-time optimized paths*`;
  }
  ...
  if (lower.includes("heat stroke") || lower.includes("emergency") || lower.includes("help")) {
    return `## 🚨 HEAT EMERGENCY — ACT NOW\n\n` +
      ...
      `⚠️ **Heat stroke is life-threatening — every minute counts.**`;
  }
