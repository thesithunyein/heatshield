...
Extreme urban heat is one of the fastest-growing climate risks in the world. In Arizona alone, heat kills 977 people every year, contributing to over 4,300 deaths since 2013 and 6,863 heat-related hospital visits in 2024. The most heartbreaking part is that 88 percent of heat victims are found outdoors because they did not have access to a cooling center or the cooling center was in the wrong place.

The core issue is that cities currently decide where to place cooling centers based on city-wide average temperatures. A city planner looks at the average temperature for an entire region and places resources wherever it looks hot on paper. But the reality is that one city block can be 110 degrees while the block right next to it is only 95 degrees. That 15-degree difference is invisible on standard weather maps, but it is the difference between life and death for vulnerable populations like unhoused individuals, elderly residents, and outdoor workers who lack reliable cooling access.

HeatShield is built for city emergency management departments, urban planners, and public health officials who need to make data-driven decisions about deploying cooling resources. Designed specifically for city planners managing mobile cooling units during heatwaves, HeatShield replaces macro-level temperature averages with street-level micro-climate data so officials can position centers for maximum life-saving impact.

HeatShield integrates with the FortyGuard Temperature API across three main endpoints. First, the Heatmap endpoint generates real-time temperature maps at 20-meter resolution so every city block receives its own real reading. Second, the Environmental Parameters endpoint gathers 24-hour data for heat index, humidity, wind speed, air quality, and UV index to feed our localized risk-scoring algorithm. Third, the Heat Intelligence endpoint provides context-aware spatial recommendations for cooling center placement.

The platform also includes a Cooling Center Optimizer that calculates unserved high-risk blocks, quantifies residents at risk, and pinpoints exact coordinates to deploy new resources. To make the system complete, we built an AI advisor that answers heat safety questions using live FortyGuard data, a cool routes planner that finds the safest walking paths using street-level temperatures, and a digital twin that simulates how urban heat shifts throughout the day.

Our analysis shows that HeatShield identifies 131 high-risk blocks in Phoenix with no cooling center coverage, putting over 1,600 residents at risk during peak heat hours. The platform recommends deploying mobile cooling units to specific locations that cover the most unserved blocks and protect vulnerable populations. Based on our modeling, optimal cooling center placement using hyperlocal data could prevent an estimated 47 heat-related deaths every summer in Phoenix alone.

The platform is live at heatshield.sithunyein.com and the source code is open-source on GitHub. HeatShield takes raw temperature data and turns it into life-saving decisions using real FortyGuard data at a resolution no other tool provides.
